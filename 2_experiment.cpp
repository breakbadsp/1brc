...
struct Station {
  ...
  int total_measurements {0};
   ...
   void Add(double temp1 ) {
    const int temp = temp1 * 1000;
...
int main(int argc, char* argv[]) {
  ...
  auto ret1 = madvise(mmf, sb.st_size, MADV_WILLNEED | MADV_SEQUENTIAL | MADV_HUGEPAGE);
  if(ret1 == -1) {
    return -1;
  }
  ...
  for(auto i = data.begin();  i != data.end(); ++i) {
    ...
    oline = city;
    oline += ';';
    oline += to_chars(st.min/1000).data();
    oline += ';';
    oline += to_chars(st.mean/1000).data();
    oline += ';';
    oline += to_chars(st.max/1000).data();
